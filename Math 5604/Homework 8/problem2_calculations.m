...
fprintf("Begin problem 2 (c)\n");

% Domain is [0,1] x [0, 2], so M = 128, N = 256 => h_x = h_y = 1/128
[x, y, u] = problem2(128, 256);

% Note that we have used the convention y-axis <-> rows, so our solution
% is compatible with meshgrid.
[xx, yy] = meshgrid(x, y);
u_ex = 2 + sin(pi*xx) .* sin(pi*yy);
...
last_l2_error = 0;
for i_h = 1:length(h)
    [x, y, u] = problem2(round(1/h(i_h)), round(2/h(i_h)));
    [xx, yy] = meshgrid(x, y);
    u_ex = 2 + sin(pi*xx) .* sin(pi*yy);
