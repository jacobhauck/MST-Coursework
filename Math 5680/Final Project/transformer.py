...
class TokenEmbedding(torch.nn.Module):
    ...
    def num_tokens(self):
        """
        :return: Total number of tokens in the embedding (including special
            tokens)
        """
    ...
    def indices(self, x: List[str]):
        """
        Convert a list of strings into a list of token index sequences.
    ...
    def unembed(self, indices, include_special: bool = False):
        """
        Convert a list of token index sequences into a string.
    ...
    def forward(self, indices):
        """
        Convert a set of token index sequences into a matrix of token
        embedding vectors.
        ...
        if mask is None:
            return torch.matmul(
                torch.nn.functional.softmax(
                    torch.matmul(
                        queries, torch.transpose(keys, 1, 2)
                    ) / (keys.size()[2] ** .5), dim=2
                ),
                values
            )
...
class Transformer(torch.nn.Module):
    ...
    def __init__(
        ...
        # Initialize layers besides encoder and decoder
    ...
    def input_attention_mask(self, input_indices):
        ...
        if self.training:
            ...
            # be masked in subsequent layers (and in cross-attention layers).
    ...
    def next_token(
        ...
        :param output_pe: (B, n_out, d) output token embedding with
            positional encoding
        :param output_mask: (B, n_out, n_out) mask for output attention
        :param cross_mask: (B, n_out, n_in) mask for cross attention
    ...
    def forward(self, x: List[str] | str, y: List[str] | None = None):
        ...
        # Convert input to indices, embed, add positional encoding, and get mask
        ...
        # Action of decoder depends on whether we are training or testing.
        if y is not None:
            # y is not None, so we are training

            # Convert ground-truth output to indices, get embedding, add
            # positional encoding
            ...
            # Get output mask for self-attention and cross-attention mask
            ...
            # make sure to drop the last probability output (there is no
            # next token after the last one)
            return prob[..., :-1], output_indices[:, 1:]

        else:
            # y is None, so we are testing.
